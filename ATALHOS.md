...
> **Referência completa de atalhos e comandos otimizados**
...
| `-k` | `--keep-artifacts` | Manter ZIPs e temporários |
| `-D` | `--create-database` | Criar banco DuckDB |
| `-K` | `--keep-parquet-after-db` | Manter parquets após DB |
...
### 🆕 Argumentos do Painel
| Atalho | Argumento Completo | Descrição |
|--------|-------------------|-----------|
| `-P` | `--processar-painel` | Ativar processamento do painel |
| (nenhum) | `--painel-uf` | Filtrar por UF (ex: SP, GO) |
| (nenhum) | `--painel-situacao` | Filtrar por situação (2=Ativa) |
| (nenhum) | `--painel-incluir-inativos` | Incluir estabelecimentos inativos |
